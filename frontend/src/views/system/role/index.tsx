...
import { Button, Form, Input, Select, Col, Row, Tooltip, Table, Pagination, Modal, Radio, DatePicker, Switch, message, Checkbox, Tree, InputNumber, Dropdown, MenuProps } from 'antd';
import {
  SyncOutlined,
  SearchOutlined,
  PlusOutlined,
...
import { getListAPI, delAPI, getDetailAPI, addAPI, putAPI, putRoleStatusAPI } from '@/api/modules/system/role';
...
const SysRole: React.FC = () => {
  ...
  const handleEditForm = async (id: number) => {
    try {
      ...
      const newSelectIds = solveAntHalfSelect(menus as menusType[], data.result.menuIds as number[], 'menuId');
  ...
  const onShowPostChange = (e: CheckboxChangeEvent) => {
    if (e.target.checked) {
      ...
      const ids: number[] = [];
      const checkChild = (list: IdeptResultType[]) => {
        list.forEach((item) => {
          if (item.children?.length) {
    ...
    },
    {
      ...
      render: (_: any, record: IroleType) => (
        <div hidden={record.roleKey === 'admin'}>
          ...
          <Button hidden={hasPermi('system:role:remove')} size="small" icon={<DeleteOutlined />} type="link" onClick={() => delFn(String(record.roleId))}>
            删除
          </Button>
          <Dropdown
            menu={{
        ...
        <Col span={24}>
          <Form form={queryForm} hidden={!searchShow} layout="inline" className="leno-search">
            ...
            <Form.Item label="权限字符" name="roleKey">
              <Input style={{ width: 240 }} placeholder="请输入权限字符" allowClear onPressEnter={searchQueryFn} />
            </Form.Item>
          ...
          <Row gutter={16} className="mb10">
            <Col span={16} className="leno-btn">
              <Row gutter={8}>
                ...
                <Col>
                  <ColorBtn hidden={hasPermi('system:role:remove')} onClick={() => delFn(rowKeys)} disabled={multiple} color="danger" icon={<DeleteOutlined />}>
                    删除
                  </ColorBtn>
                </Col>
                <Col>
          ...
          >
            ...
            >
              <Form.Item label="角色名称" name="roleName" hidden={false} rules={[{ required: true, message: '请输入角色名称!' }]}>
                <Input placeholder="请输入角色名称" />
              </Form.Item>
              <Form.Item label="权限字符" name="roleKey" hidden={false} rules={[{ required: true, message: '请输入权限字符!' }]}>
                <Input placeholder="请输入权限字符" />
              </Form.Item>
              <Form.Item label="显示顺序" name="roleSort" hidden={false} rules={[{ required: true, message: '请输入显示顺序!' }]}>
                <InputNumber min={0} />
              </Form.Item>
              <Form.Item label="状态" name="status">
                <Radio.Group
          ...
          <Modal
            ...
            forceRender
          >
            <Form form={rolePermForm} labelCol={{ span: 5 }} onFinish={handleRoleFormFinish} initialValues={{ dataScope: '1' }}>
              <Form.Item label="角色名称" name="roleName" hidden={false} rules={[{ required: true, message: '请输入角色名称!' }]}>
                <Input placeholder="请输入角色名称" />
              </Form.Item>
              <Form.Item label="权限字符" name="roleKey" hidden={false} rules={[{ required: true, message: '请输入权限字符!' }]}>

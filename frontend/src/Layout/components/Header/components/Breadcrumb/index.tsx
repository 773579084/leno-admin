...
const BreadcrumbCom = () => {
  ...
  useEffect(() => {
    let route: RouteType = { element: undefined };
  ...
  return (
    <Breadcrumb className={classes['bread-crumb']} style={{ marginLeft: 16 }}>
      <Breadcrumb.Item className={pathname === HOME_URL ? classes['current-home'] : classes.home} onClick={() => navigate(HOME_URL)}>
        首页
      </Breadcrumb.Item>
      {pathname === HOME_URL
        ? null
        : newBreadcrumbArr.map((item, index) => (
            <Breadcrumb.Item className={newBreadcrumbArr.length - 1 === index ? classes['current-bread'] : classes['nocurrent-bread']} key={index}>
              {item}
            </Breadcrumb.Item>
          ))}
    </Breadcrumb>

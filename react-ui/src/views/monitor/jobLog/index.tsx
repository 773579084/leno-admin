...
const MonitorJobLog = () => {
  ...
  const rowSelection = {
    ...
    onChange: (selectedRowKeys: React.Key[], selectedRows: IjobLogType[]) => {
  ...
  let columns = [
    ...
    },
    {
      ...
      render: (_: any, record: IjobLogType) => (
        <div>
          <Button
            hidden={hasPermi('monitor:jobLog:remove')}
            size="small"
            icon={<DeleteOutlined />}
            type="link"
            onClick={() => delFn(String(record.jobLogId))}
          >
            删除
          </Button>
        </div>
          ...
          <Row gutter={16} className="mb10">
            <Col span={16} className="leno-btn">
              <Row gutter={8}>
                ...
                <Col>
                  <ColorBtn
                    hidden={hasPermi('monitor:jobLog:export')}
                    color="warning"
                    icon={<VerticalAlignBottomOutlined />}
                    onClick={() => download('/monitor/jobLog/export', 'sys_dict_type')}
                  >
                    导出
                  </ColorBtn>
                </Col>

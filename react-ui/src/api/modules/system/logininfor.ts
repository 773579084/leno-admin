...
export const getListAPI = (data: IlogininforType) => {
  return http<IgetListAPI>('GET', '/system/logininfor/list', data)
}

// 删除
export function delAPI(ids: string) {
  return http<IsuccessTypeAPI>('DELETE', '/system/logininfor/' + ids)
}

// 清空
export function cleanAPI() {
  return http<IsuccessTypeAPI>('DELETE', '/system/logininfor/clean')
}

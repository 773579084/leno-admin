import Router from 'koa-router'
// 格式转换
import { formatHandle } from '@/business/middleware/common/common.middleware'
import IndexCon from '@/business/controller'
import {
  getListMid,
  getAddMid,
  getDetailMid,
  putMid,
  delMid,
  exportMid
} from '@/business/middleware/system/user.middleware'
import { addEditSchema, judgeIdSchema } from '@/business/schema'
import { exportExcelMid } from '@/business/middleware/common/common.middleware'
import LenoUser from '@/mysql/model/system/user.model'
import { exportExcelSer } from '@/business/service'
import { addJudg, putJudg } from '@/business/schema/system/user.schema'
import { hasPermi } from '@/business/middleware/common/auth'

const router = new Router({ prefix: '/system' })
// 查询列表
router.get('/user/list', hasPermi('system:user:query'), getListMid, formatHandle, IndexCon())

// 新增
router.post(
  '/user',
  hasPermi('system:user:add'),
  addEditSchema(addJudg),
  getAddMid,
  IndexCon()
)

// 删除
router.delete('/user/:id', hasPermi('system:user:remove'), judgeIdSchema(), delMid, IndexCon())

// 获取详细数据
router.get('/user/detail/:id', hasPermi('system:user:query'), judgeIdSchema(), getDetailMid, formatHandle, IndexCon())

// 修改
router.put(
  '/user',
  hasPermi('system:user:edit'),
  addEditSchema(putJudg),
  putMid,
  IndexCon()
)

// 导出列表(excel)
router.post(
  '/user/export',
  hasPermi('system:user:export'),
  exportExcelMid(exportExcelSer, LenoUser, {}),
  exportMid,
  IndexCon()
)
module.exports = router